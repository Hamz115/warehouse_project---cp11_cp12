...
public:
  ...
  ApproachShelf() : Node("approach_service_server") {
    ...
    _laser_subscription =
        this->create_subscription<LaserScan>("/scan", 1, laser_callable);
    // Create the service server
    auto callable_handle =
        std::bind(&ApproachShelf::handle_service, this, _1, _2);
    _service =
        this->create_service<GoToLoading>("approach_shelf", callable_handle);
...
private:
  ...
  // Methods
  void handle_service(const std::shared_ptr<GoToLoading::Request>,
                      std::shared_ptr<GoToLoading::Response> response) {
    // Handle request
    RCLCPP_INFO(this->get_logger(), "Legs detected: %i.", _num_legs);
    RCLCPP_INFO(this->get_logger(), "L: %i R: %i.", _leg_data[0].first,
                _leg_data[1].first);
    ...
    if (_num_legs == 2) {
      ...
      if (!this->approach_cart("robot_front_laser_link", "cart_frame")) {
        RCLCPP_INFO(this->get_logger(),
                    "The robot did not aligned to the cart correctly.");
      ...
      if (!this->approach_cart("robot_front_laser_link", "cart_center")) {
        RCLCPP_INFO(this->get_logger(),
                    "The robot did not get under the cart correctly.");
        response->complete = false;
        return;
      }
  ...
  void publish_cart_frame() {
    ...
    float theta =
        std::abs(_leg_data[0].first - _leg_data[1].first) * _scan_increment;
    // C: AB (Law of Cosines)
    ...
    do {
      ...
      try {
        ...
        odom_to_laser = _tf_buffer->lookupTransform(parent_fame, laser_frame,
                                                    tf2::TimePointZero);
        ...
        RCLCPP_INFO(this->get_logger(),
                    "Could not perform some of the transformations. Please "
    ...
    do {
      ...
      try {
        ...
        while (rclcpp::ok() && elapsed_time < timeout) {
          ...
          RCLCPP_DEBUG(this->get_logger(), "Elapsed time: %.2f seconds",
                       elapsed_time.seconds());
          geometry_msgs::msg::TransformStamped t;
          t = _tf_buffer->lookupTransform(origin_frame, target_frame,
                                          tf2::TimePointZero);
          // Compute velocities
          ...
          if (error_distance > 0.02) {
            //  Set velocity
            if (error_yaw > 0.0 && error_distance > 0.025) {
              vel_msg.angular.z = (-1.0 * error_yaw) - 0.05;
            } else if (error_yaw < 0.0 && error_distance > 0.025) {
              vel_msg.angular.z = (-1.0 * error_yaw) + 0.02;
            } else {
              vel_msg.angular.z = 0;
            }
            vel_msg.linear.x = std::min(2.0 * error_distance, 0.1);
            RCLCPP_INFO(this->get_logger(), "Z angular: %.3f",
                        vel_msg.angular.z);
            RCLCPP_INFO(this->get_logger(), "X linear: %.3f", vel_msg.linear.x);
            // Pubish velocity
            ...
            vel_msg.linear.x = 0;
            _publisher->publish(vel_msg);
            RCLCPP_INFO(this->get_logger(), "Approach to %s completed.",
                        target_frame.c_str());
            completed = true;
            break;
          }
